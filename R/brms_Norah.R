...
p <- ggplot(datxy,aes(y=diff, x=as.factor(LU_GROUPING3)))
...
aggregate(datxypeak$CREEL,by=list(datxypeak$LU_GROUPING3),propna)
...
#Just plot 2020
p <- ggplot(datnna %>% filter(YEAR==2018),aes(y=CREEL, x=IREC,color=as.factor(LU_GROUPING3)))
p <- p + geom_point(size=2, alpha=.5)
p <- p + geom_smooth(method = lm, formula= y~0+x, se     = FALSE, size   = 1, alpha  = .8) # to add regression line
p <- p + theme_bw(16)+labs( y="CREEL", x="iREC")
p <- p + scale_color_viridis_d(end = 0.8,option = "B")
...
#============================= Histograms
datgath<- datnna %>% pivot_longer(names_to ="grp", values_to ="estimate", c(IREC, CREEL))
