...
#===========================================
#plot data by area
p <- ggplot(datnna,aes(y=log(CREEL+1), x=log(IREC+1),color=LU_GROUPING3))
...
datnna$creellogic<-datnna$CREEL>0
p <- ggplot(datnna,aes(y=creellogic, x=log(IREC+1),color=LU_GROUPING3))
p <- p + geom_point(size=2, alpha=.5, position=position_jitter(width=0.05,height=.1))
p <- p + geom_smooth(method = lm, formula= y~0+x, se     = FALSE, size   = 1, alpha  = .8) # to add regression line
p <- p + theme_bw(16)+labs( y="CREEL", x="iREC")
p <- p + scale_color_viridis_d(end = 0.8,option = "C")
p <- p + theme(legend.position="bottom")
...
#plot data by year
p <- ggplot(datnna,aes(y=log(CREEL+1), x=log(IREC+1),color=as.factor(YEAR)))
p <- p + geom_point(size=2, alpha=.5)
p <- p + geom_smooth(method = lm, formula= y~0+x, se     = FALSE, size   = 1, alpha  = .8) # to add regression line
...
#===============================
#TOdo exploratory analyses to see the sahpe of the mean respose

#1 fit logistic regressions with glmmtmb
#logiccreel ~log(IREC+1)
...
#2 fit the model with neg binomial model
