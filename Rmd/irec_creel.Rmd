...
# Do not run when rendering as the results are not meaningful and it takes a loooong time to run
 fit1 <- brm(formula= IREC |se(SDIREC,sigma = TRUE) ~ -1 +  (-1 + CREEL |LU_GROUPING3) , 
  data=datxy, family=gaussian, iter = 200,chains=2)

fit2 <- brm(formula= IREC |se(SDIREC,sigma = TRUE) ~ -1 +  ( CREEL |LU_GROUPING3) , 
  data=datxy, family=gaussian, iter = 200,chains=2)

(mean(datxy$SDIREC[datxy$SDIREC>0]))

#add measurement error as latent variable - mi does not accept NA or zero variances
#Infill SDIREC




min(datxy$IREC[!is.na(datxy$SDIREC)&datxy$IREC>0])
summary(datxynna)
bform <- bf(IREC | mi(SDIREC) ~ -1 +  (-1 + CREEL |LU_GROUPING3))
fitmi <- brm(formula= bform , data=datxynna, family=gaussian, iter = 2000,chains=2)
