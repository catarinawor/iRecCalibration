...
This version contains qualty control for creel data. 
...
creelc<- creel %>%
  ...
  rename(AREA=PFMA, SD=STANDARD_ERROR,DISPOSITION=TYPE)%>%
  select(c(AREA,YEAR,MONTH,DISPOSITION,ESTIMATE,SD))%>%
  mutate(SURVEY="creel")

creelc15<- creel %>%
  filter(ESTIMATE_SOURCE=="Creel",  Include..15.=="Y")%>%
  rename(AREA=PFMA, SD=STANDARD_ERROR,DISPOSITION=TYPE)%>%
  select(c(AREA,YEAR,MONTH,DISPOSITION,ESTIMATE,SD))%>%
  mutate(SURVEY="creel15")

creelc20<- creel %>%
  filter(ESTIMATE_SOURCE=="Creel",  Include..20.=="Y")%>%
  rename(AREA=PFMA, SD=STANDARD_ERROR,DISPOSITION=TYPE)%>%
  select(c(AREA,YEAR,MONTH,DISPOSITION,ESTIMATE,SD))%>%
  mutate(SURVEY="creel20")

dat<-rbind(creelc,creelc15,creelc20,irecc)
...
creelf<- creel %>%
  ...
  TRUE ~ "creel"))%>%
  rename(AREA=PFMA, SD=STANDARD_ERROR,DISPOSITION=TYPE)%>%
  select(c(AREA,YEAR,MONTH,DISPOSITION,ESTIMATE,SD, SURVEY))
...
p <- ggplot(datxy)
p <- p + geom_point(aes(x=CREEL, y=IREC,color=SURVEY, shape=SURVEY), size=2,alpha=.5)
...
p <- ggplot(datxy, aes(x=as.factor(AREA),y=ratio, color=SURVEY))
p <- p + geom_boxplot(width=1, position = position_dodge(width = 1))
p <- p +geom_hline(aes(yintercept=1),size=1.1, alpha=.5)
p <- p + scale_color_viridis_d(end = 0.8,option = "A")
p <- p + theme_bw(16)+labs( x="month", y="iRec:creel")
...
p2 <- ggplot(datxy, aes(x=as.factor(AREA),y=ratio, color=SURVEY))
p2 <- p2 + geom_boxplot(width=1, position = position_dodge(width = 1))
p2 <- p2 +geom_hline(aes(yintercept=1),size=1.1, alpha=.5)
p2 <- p2 + scale_color_viridis_d(end = 0.8,option = "A")
p2 <- p2 + theme_bw(16)+labs( x="month", y="iRec:creel")
